...
function App() {
  ...
  return <main className="app-shell">
    ...
    {session.user.demo && <section className="system-info"><div className="system-info-heading"><div><p className="eyebrow">Portfolio demonstration</p><h2>How the system works</h2></div><p>This view uses safe sample data to demonstrate the complete Smart Drainage workflow.</p></div><div className="system-flow"><article><span>1</span><h3>Detect</h3><p>The JSN-SR04T ultrasonic sensor measures debris level while the float switch checks for overflow.</p></article><article><span>2</span><h3>Process</h3><p>The ESP32 filters readings, calculates the fill percentage, and controls the local green, yellow, or red LED.</p></article><article><span>3</span><h3>Send</h3><p>Current readings travel through Wi-Fi using HTTPS to the Node.js backend hosted on Render.</p></article><article><span>4</span><h3>Sync</h3><p>The backend saves current data in Supabase and sends live updates to open browser dashboards.</p></article><article><span>5</span><h3>Respond</h3><p>The dashboard shows each unit’s condition and provides browser voice and alarm alerts for critical, full, or overflow states.</p></article></div><div className="system-details"><article><h3>Fill calculation and calibration</h3><p>An administrator calibrates a unit while its debris container is empty. The recorded empty distance becomes the zero reference, while 21 cm is the fixed full-point value. The dashboard displays the calculated percentage, not raw sensor distance.</p></article><article><h3>Status and connection</h3><p>Normal is below 40%, warning is 40–74%, and critical/full is 75% or higher. An overflow reading has priority. A unit is shown as disconnected when it has no recent reading; it returns to live once new data arrives.</p></article><article><h3>Demo mode note</h3><p>All readings, user accounts, additions, deletions, and calibrations in this mode are temporary. Demo mode does not contact or modify your real device, backend, or Supabase database.</p></article></div></section>}
