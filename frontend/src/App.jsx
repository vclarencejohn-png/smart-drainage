...
function App() {
  ...
  return <main className="app-shell">
    ...
    <section className="dashboard-panel">
      <div className="tabs dashboard-tabs"><button className={dashboardTab === 'overall' ? 'active' : ''} onClick={() => { setDashboardTab('overall'); setExpandedId(null); }}>Overall updates</button><button className={dashboardTab === 'drainages' ? 'active' : ''} onClick={() => setDashboardTab('drainages')}>Drainages</button></div>
      {dashboardTab === 'overall' ? <section className="overall-updates"><div className="overall-heading"><div><p className="eyebrow">Live summary</p><h2>Overall updates</h2></div><p>See the current condition of every registered drainage unit at a glance.</p></div><div className="overall-grid">{overallGroups.map((group) => <article className={`overall-status-card ${group.key}`} key={group.key}><div className="overall-card-title"><h3>{group.label}</h3><strong>{group.items.length}</strong></div><p>{group.description}</p>{group.items.length ? <ul className="overall-status-list">{group.items.map(({ unit, reading, status }) => <li key={unit.id}><span><strong>{unit.name}</strong><small>{reading ? `${Math.round(reading.debris_level)}%` : 'Awaiting first reading'}</small></span><b>{status.label}</b></li>)}</ul> : <p className="overall-empty">No units in this condition.</p>}</article>)}</div></section> : <section className={`drainage-grid ${expandedId ? 'expanded' : ''}`}>
