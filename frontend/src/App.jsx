...
# Build the complete updated App.jsx with the two changes

app_jsx = r'''import React, { useState, useEffect, useRef, useCallback } from 'react';
...
const translations = {
  en: {
    ...
    live: 'Live', offline: 'Offline', maintenance: 'Maintenance', startMaintenance: '\u{1F527} Start Maintenance',
    endMaintenance: '\u{2705} End Maintenance', underMaintenance: '\u{1F527} Under Maintenance', maintenanceBy: 'By',
    ...
    tagalog: 'Tagalog', voiceAlert: 'Voice Alert', enablePush: '\u{1F515} Enable Alerts', disablePush: '\u{1F514} Disable Alerts',
  ...
  tl: {
    ...
    live: 'Live', offline: 'Offline', maintenance: 'Maintenance', startMaintenance: '\u{1F527} Simulan ang Maintenance',
    endMaintenance: '\u{2705} Tapos na ang Maintenance', underMaintenance: '\u{1F527} Sa Ilalim ng Maintenance', maintenanceBy: 'Ni',
    ...
    voiceAlert: 'Voice Alert', enablePush: '\u{1F515} Buksan ang Alerts', disablePush: '\u{1F514} Isara ang Alerts',
...
function App() {
  ...
  // ========== AUTO-SUBSCRIBE PUSH NOTIFICATIONS ON LOGIN ==========
  const autoSubscribePush = useCallback(async () => {
    ...
    } catch (e) { console.error('Auto push subscribe error:', e); }
  }, []);
  ...
  useEffect(() => {
    ...
    s.on('alert', (alert) => {
      if (alert.unit_id === selectedUnit) {
        ...
        // \u{1F50A} ALWAYS play alarm sound + voice on overflow/critical (no toggle needed)
  ...
  // \u{1F50A} Alarm sound using Web Audio API (always plays on overflow/critical)
    ...
    return (
      <div className={`login-container ${darkMode ? 'dark' : ''}`}>
        <div className="login-box">
          <h1>\u{1F30A} Smart Drainage</h1>
          <h2>{t.login}</h2>
          {!isOnline && <div className="offline-banner">\u{26A0}\u{FE0F} You are offline. Some features may not work.</div>}
          ...
          <div className="login-settings">
            <button onClick={() => setDarkMode(!darkMode)}>{darkMode ? '\u{2600}\u{FE0F}' : '\u{1F319}'}</button>
            <button onClick={() => setLang(lang === 'en' ? 'tl' : 'en')}>{lang === 'en' ? '\u{1F1F5}\u{1F1ED} TL' : '\u{1F1FA}\u{1F1F8} EN'}</button>
          </div>
        </div>
      </div>
    );
    ...
    <div className={`app ${darkMode ? 'dark' : ''}`}>
      ...
      <header className="app-header">
        ...
        <div className="header-controls">
          <span className="timer">\u{23F1}\u{FE0F} {t.willAutoLogout}: {formatTimeLeft(timeLeft)}</span>
          ...
          <button onClick={() => setActiveTab('settings')}>\u{2699}\u{FE0F}</button>
          <button onClick={handleLogout}>{t.logout}</button>
        </div>
      </header>

      <nav className="app-nav">
        <button className={activeTab === 'dashboard' ? 'active' : ''} onClick={() => setActiveTab('dashboard')}>\u{1F4CA} {t.dashboard}</button>
        <button className={activeTab === 'history' ? 'active' : ''} onClick={() => setActiveTab('history')}>\u{1F4C8} {t.history}</button>
        <button className={activeTab === 'notifications' ? 'active' : ''} onClick={() => setActiveTab('notifications')}>\u{1F514} {t.notifications}</button>
        {user?.role === 'admin' && <button className={activeTab === 'admin' ? 'active' : ''} onClick={() => setActiveTab('admin')}>\u{1F510} {t.adminPanel}</button>}
      </nav>

      {activeTab === 'dashboard' && (
        <div className="dashboard">
          {!isOnline && <div className="offline-banner">\u{26A0}\u{FE0F} Offline mode - Showing cached data</div>}
          {maintenanceStatus[selectedUnit]?.active && (
            <div className="maintenance-banner">
              \u{1F527} {t.underMaintenance} 
          ...
          <div className="status-cards">
            <div className="status-card">
              ...
              {sensorData?.maintenance && <span className="maintenance-badge">\u{1F527}</span>}
            </div>
            <div className="status-card">
              <h3>Distance</h3>
            ...
            <div className="status-card">
              ...
              <div className={`big-number ${sensorData?.overflow ? 'danger' : 'safe'}`}>
                {sensorData ? (sensorData.overflow ? 'YES \u{1F6A8}' : 'NO \u{2713}') : '--'}
              </div>
            </div>
            <div className="status-card">
              <h3>{t.ledStatus}</h3>
      ...
      {activeTab === 'history' && (
        <div className="history-tab">
          ...
          <div className="history-filters">
            ...
            <button onClick={downloadCSV}>\u{1F4C4} {t.downloadCSV}</button>
            <button onClick={downloadPDF}>\u{1F5A8}\u{FE0F} {t.downloadPDF}</button>
          </div>
      ...
      {activeTab === 'notifications' && (
        <div className="notifications-tab">
          ...
          <button onClick={fetchNotificationLog} className="refresh-btn">\u{1F504} {t.refresh}</button>
              ...
              safeArray(notificationLog).slice().reverse().map((n, i) => (
                <div key={i} className={`notification-item ${n?.type}`}>
                  <div className="notification-header">
                    ...
                    <span className="notification-type">{n?.type === 'overflow' ? '\u{1F6A8}' : '\u{26A0}\u{FE0F}'} {n?.type?.toUpperCase()}</span>
                    <span className="notification-time">{n?.time ? new Date(n.time).toLocaleString() : '-'}</span>
                  </div>
                  <p className="notification-message">{n?.message}</p>
                  <span className="notification-sent">{n?.sent ? '\u{2713} Delivered' : '\u{2717} Not delivered'}</span>
                </div>
              ))
            )}
          </div>
      ...
      {activeTab === 'admin' && user?.role === 'admin' && (
        <div className="admin-panel">
          <h2>{t.adminPanel}</h2>
          {adminError && <div className="error-banner">\u{26A0}\u{FE0F} {adminError}</div>}
          
          <div className="admin-section">
            <h3>\u{1F465} {t.users}</h3>
            <button onClick={() => setAddUserModal(true)} className="refresh-btn" style={{background: '#4caf50', marginBottom: '16px'}}>\u{2795} {t.addUser}</button>
          ...
          <div className="admin-section">
            <h3>\u{1F4CD} {t.units}</h3>
            <button onClick={() => setAddUnitModal(true)} className="refresh-btn" style={{background: '#4caf50', marginBottom: '16px'}}>\u{2795} {t.addUnit}</button>
            <div className="units-list">
              {safeArray(units).map(u => (
                <div key={u?.unit_id || Math.random()} className="unit-card">
                  ...
                  {maintenanceStatus[u?.unit_id]?.active && <span className="maintenance-badge-small">\u{1F527} {t.maintenance}</span>}
                  <button 
          ...
          <div className="admin-section">
            <h3>\u{1F4CB} {t.loginHistory}</h3>
            <button onClick={fetchLoginHistory} className="refresh-btn">\u{1F504} {t.refresh}</button>
            <table className="admin-table">
          ...
          <div className="admin-section">
            <h3>\u{1F527} Maintenance Status</h3>
      ...
      {activeTab === 'settings' && (
        <div className="settings-panel">
          <h2>\u{2699}\u{FE0F} {t.settings}</h2>
          <div className="setting-item">
            <label>{t.darkMode}</label>
            <button onClick={() => setDarkMode(!darkMode)}>{darkMode ? '\u{2600}\u{FE0F} ' + t.lightMode : '\u{1F319} ' + t.darkMode}</button>
          </div>
          <div className="setting-item">
            <label>{t.language}</label>
            <button onClick={() => setLang(lang === 'en' ? 'tl' : 'en')}>{lang === 'en' ? '\u{1F1F5}\u{1F1ED} ' + t.tagalog : '\u{1F1FA}\u{1F1F8} ' + t.english}</button>
          </div>
          <button onClick={() => setActiveTab('dashboard')} className="back-btn">\u{2190} Back to Dashboard</button>
        </div>
      )}
      ...
      {maintenanceModal && (
        <div className="modal-overlay">
          <div className="modal">
            <h3>\u{1F527} {t.startMaintenance}</h3>
...
print(f"File size: {len(app_jsx)} characters")
